...
class HMM():
    ...
    def xi_comp(self, alpha, beta, gamma):
        ...
        #beta --> beta_k, alpha --> alpha_k, 
        # eta = np.divide(gamma, np.multiply(alpha, beta))
       ...
       # print("ETA",eta.shape)
        ...
        for k in range(self.Y.shape[0]-1):
           ...
           # xi[k,:] = np.multiply(eta, np.outer(np.multiply(alpha[k,:],self.T),np.multiply(self.M[:,a],beta[k+1,:])))
